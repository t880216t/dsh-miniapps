...
- **顶栏随窗口顶部归属变化**：谁是窗口最顶上那一行，谁负责给系统按钮让位、负责拖窗。桌面壳的 `compatibility` / `extended` 模式在窗口顶部有一条 36px 的产品框架（读 `<body>` 上的 `data-dsh-desktop-mode`），此时顶栏是第二行——30px、左右 6px、不拖窗、不给红绿灯或标题按钮留位；`advanced` 模式或纯浏览器里顶栏自己就是最顶行——44px，macOS 左侧让出 88px 给红绿灯，Windows 右侧让出 146px 给标题按钮，空白处拖窗。
- **常驻与收起**：覆盖层首次打开后**挂载一次、之后只隐藏**，因此每个打开过的小程序都保留页面、滚动位置与会话；顶栏的 × 是「收起」。每个已配置的小程序是一个**带图标和名字的标签**：没打开过的半透明、点一下即打开；已打开的悬停时出现 ×，当前的常驻 ×。关闭单个小程序会丢弃它的页面，下次打开从首页开始。没有任何小程序时内容区居中显示一张引导卡，按钮直达「管理」。
